...
class WatermarkStore:
    ...
    def get_last_seen_id(self) -> int | None:
        with self._conn() as con:
            row = con.execute(
    ...
    def get_last_sync_completed(self) -> datetime | None:
        with self._conn() as con:
            row = con.execute(
    ...
    def get_backfill_cursor(self) -> int | None:
        with self._conn() as con:
            row = con.execute(
    ...
    def get_backfill_peak_id(self) -> int | None:
        with self._conn() as con:
            row = con.execute(
    ...
    def save_backfill_progress(self, skip: int, peak_id: int) -> None:
        ...
        Both keys are written in one transaction: a crash between them would
        otherwise leave the cursor advanced with a stale peak, shrinking the
        final watermark and making the next incremental sync re-fetch pages.
        """
        with self._conn() as con:
            con.begin()
            try:
